...
import { Preview } from "./components/comp.preview";
      ...
      <section className="m-auto my-12 grid max-w-7xl grid-cols-3 gap-12">
                  ...
                  {section.lessons.map((lesson) => {
                    return (
                      <Card key={lesson.id} className="flex items-center justify-between p-4">
                        ...
                        {lesson.isPreview && <Preview videoUrl={lesson.videoUrl}></Preview>}
                      </Card>
                    );
                  })}
                </div>
        ...
        <Card className="-mt-32 h-fit space-y-4 bg-white p-4">
          ...
          <form action={buyCourseAction}>
            ...
            <input type="hidden" name="amount" value={course.flashSales ? course.flashSales.newAmount : course.price} required />
